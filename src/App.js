    ...
    <>
      ...
      <Main>
        <Box>
          ...
          {isLoading && <Loader />}
          {!isLoading && !error && (
            <MovieList movies={movies} onMovieHandler={onMovieHandler} />
          )}
...
*/
const MovieList = ({ movies, onMovieHandler }) => {
  return (
...
const Movie = ({ movie, onMovieHandler }) => {
  return (
    <li onClick={() => onMovieHandler(movie.imdbID)}>
          ...
          <section>
            <div className="rating">
              {!isWatched ? (
                ...
                <p>
                  This rated this movie {watchedUserRating} <span>⭐</span>
                </p>
              )}
            </div>
            <p>
...
const WatchedMoviesList = ({ watched, handleWatchedDelete }) => {
  return (
    <ul className="list">
      {watched.map((movie) => (
...
const WatchedMovie = ({ movie, handleWatchedDelete }) => {
  return (
    <li key={movie.imdbID}>
      <img src={movie.poster} alt={`${movie.title} poster`} />
      <h3>{movie.title}</h3>
